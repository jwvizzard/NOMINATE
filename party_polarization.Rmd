...
```{r}
head(polar_dat)
write_csv(polar_dat,path="/Users/james-macbook/Library/CloudStorage/ProtonDrive-james.vizzard@proton.me-folder/James/201801-20YYMM_PhD/P_Dissertation/data/NOMINATE/voteview_polarization_data.csv")
```
...
polarized_plot <- function(chamb) {
  ...
  gg <- ggplot(data=pdatl,
               ...
               scale_x_continuous(expand = c(0.15, 0), 
                                 breaks=seq(1991, max(pdatl$year), by=2)) +
              ...
              scale_color_manual(values=c("REP"="#c70828","DEM"="#1372ad"),guide="none") +
              theme_bw()
  gg
}
```

###Include Armed Services Committee

# How do I add in the Armed Services Committees? I could replace the data for Northern Republicans and Southern Democrats with Armed Services Data, and then replace the plots with Armed Services plots.
...
gg <- ggplot(polar_dat_long %>% 
              ...
              scale_x_continuous(expand = c(0.10, 0), breaks=seq(1991,max(polar_dat_long$year),by=2)) +
              geom_line() + geom_point(size=0.7) + 
